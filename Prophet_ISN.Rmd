...
```{r}
sidc <-fread("../DATA//SN_d_tot_V2.0.csv")
...
colnames(isn_monthly) <- c("ds","y")
df <- isn_monthly

m <- prophet(seasonality.mode="multiplicative")
m <- add_seasonality(m, name="cycle_11year", period=11*12,fourier.order=5)
m <- fit.prophet(m, df)
future <- make_future_dataframe(m,periods=500,freq="month")
forecast <- predict(m, future)
```
```{r}
